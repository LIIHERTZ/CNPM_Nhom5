...
public class LoginController extends HttpServlet {
	...
	private void checkLogin(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Person user = accService.login(email, password);
		if (user == null) {
			...
			if ("on".equals(req.getParameter("remember"))) {
				saveRemeberMe(resp, user.getPerID());
			}
	...
	private void waiting(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session != null && session.getAttribute("email") != null) {
			Person user = (Person) session.getAttribute("email");
			String url = "home";
			/*
			 * if (user.getRole() == 0) url = "/home"; else if (user.getType() == 1) url =
			 * "/sellerInfor"; else if (user.getType() == 2) url = "/shipper-list-shipping";
			 * else if (user.getType() == 3) url = "/adminAccount";
			 */
